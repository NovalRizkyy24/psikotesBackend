function transformLeadershipStyle(userResult, interpretation) {
    if (!userResult || !interpretation) {
        throw new Error('Data user atau interpretasi untuk Leadership Style tidak ditemukan');
    }
    ...
    section.blocks.push({
        ...
        content: {
            mode: 'top',
            groups: [{
                name: (interpretation.results.topN && interpretation.results.topN.name) || 'Top Leadership Style', 
                items: [{
                    label: wording.title, 
                    value: 1, 
                    level: 'high',
